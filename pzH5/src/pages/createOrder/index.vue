<script setup>
import { computed, onMounted, ref } from 'vue'
import { useRouter } from 'vue-router'
import { showToast } from 'vant'
import qrcode from 'qrcode'
import api from '@/api/index.js'
import StatusBar from '@/components/statusBar.vue'
import AiDemandAssistant from '@/components/AiDemandAssistant.vue'

defineOptions({
  name: 'CreateOrderPage',
})

const router = useRouter()

const createInfo = ref({
  companion: [],
  hospitals: [],
  service: {},
})

const form = ref({
  hospital_id: '',
  hospital_name: '',
  starttime: 0,
  receiveAddress: '',
  companion_id: 0,
  demand: '',
  tel: '',
})

const showHospitals = ref(false)
const showStartTime = ref(false)
const showCompanion = ref(false)
const showCode = ref(false)

const currentDate = ref('')
const companionName = ref('')
const codeImage = ref('')
const minDate = ref(new Date())

const aiContext = computed(() => ({
  hospitalName: form.value.hospital_name,
  visitTime: currentDate.value,
  companionName: companionName.value,
  serviceName: createInfo.value.service.serviceName,
}))

const hospitalColumns = computed(() =>
  createInfo.value.hospitals.map((item) => ({
    text: item.name,
    value: item.id,
  })),
)

const companionColumns = computed(() =>
  createInfo.value.companion.map((item) => ({
    text: item.name,
    value: item.id,
  })),
)

onMounted(async () => {
  const data = await api.h5Companion()
  Object.assign(createInfo.value, data.data.data)
})

const goBack = () => {
  router.go(-1)
}

const handleHospitalConfirm = (item) => {
  form.value.hospital_id = item.selectedValues[0]
  form.value.hospital_name = item.selectedOptions[0].text
  showHospitals.value = false
}

const handleTimeConfirm = (item) => {
  const dateStr = item.selectedValues.join('-')
  form.value.starttime = new Date(dateStr).getTime()
  currentDate.value = dateStr
  showStartTime.value = false
}

const handleCompanionConfirm = (item) => {
  form.value.companion_id = item.selectedValues[0]
  companionName.value = item.selectedOptions[0].text
  showCompanion.value = false
}

const handleApplyAiDraft = (result) => {
  form.value.demand = result.demandDraft
  showToast({
    message: 'AI 建议已填入服务需求',
    position: 'middle',
  })
}

const closeCode = () => {
  showCode.value = false
  router.push('/order')
}

const submit = async () => {
  const requiredFields = [
    'hospital_id',
    'hospital_name',
    'starttime',
    'receiveAddress',
    'companion_id',
    'demand',
    'tel',
  ]

  for (const key of requiredFields) {
    if (!form.value[key]) {
      showToast({
        message: '请把每一项都填写完整',
        position: 'middle',
      })
      return
    }
  }

  const orderRes = await api.createOrder(form.value)
  qrcode.toDataURL(orderRes.data.data.wx_code).then((res) => {
    showCode.value = true
    codeImage.value = res
  })
}
</script>

<template>
  <div class="container">
    <div class="header">
      <van-icon @click="goBack" name="arrow-left" class="header-left" size="30"></van-icon>
      <span>填写订单服务</span>
    </div>

    <StatusBar item="0" />

    <van-cell class="cell">
      <template #title>
        <van-image :src="createInfo.service.serviceImg" width="25" height="25" />
        <span class="server-name">{{ createInfo.service.serviceName }}</span>
      </template>
      <template #default>
        <div class="service-text">服务内容</div>
      </template>
    </van-cell>

    <van-cell-group class="cell">
      <van-cell>
        <template #title>就诊医院</template>
        <template #default>
          <div @click="showHospitals = true">
            {{ form.hospital_name || '请选择就诊医院' }}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>

      <van-cell>
        <template #title>就诊时间</template>
        <template #default>
          <div @click="showStartTime = true">
            {{ currentDate || '请选择就诊时间' }}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>

      <van-cell>
        <template #title>陪诊员</template>
        <template #default>
          <div @click="showCompanion = true">
            {{ companionName || '请选择陪诊员' }}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>

      <van-cell>
        <template #title>接送地址</template>
        <template #default>
          <van-field
            v-model="form.receiveAddress"
            placeholder="请填写接送地址"
            class="text"
            input-align="right"
          />
        </template>
      </van-cell>

      <van-cell>
        <template #title>联系电话</template>
        <template #default>
          <van-field
            v-model="form.tel"
            placeholder="请填写联系电话"
            class="text"
            input-align="right"
          />
        </template>
      </van-cell>
    </van-cell-group>

    <AiDemandAssistant :context="aiContext" @apply="handleApplyAiDraft" />

    <van-cell-group title="服务需求" class="cell">
      <van-field
        v-model="form.demand"
        type="textarea"
        rows="4"
        autosize
        class="text"
        placeholder="请简单描述就诊科室、陪诊需求和注意事项"
      />
    </van-cell-group>

    <van-button @click="submit" class="submit" type="primary" size="large">提交订单</van-button>

    <van-popup v-model:show="showHospitals" position="bottom" :style="{ height: '30%' }">
      <van-picker
        title="医院"
        :columns="hospitalColumns"
        @confirm="handleHospitalConfirm"
        @cancel="showHospitals = false"
      />
    </van-popup>

    <van-popup v-model:show="showStartTime" position="bottom" :style="{ height: '30%' }">
      <van-date-picker
        title="选择日期"
        :min-date="minDate"
        @confirm="handleTimeConfirm"
        @cancel="showStartTime = false"
      />
    </van-popup>

    <van-popup v-model:show="showCompanion" position="bottom" :style="{ height: '30%' }">
      <van-picker
        title="陪诊员"
        :columns="companionColumns"
        @confirm="handleCompanionConfirm"
        @cancel="showCompanion = false"
      />
    </van-popup>

    <van-dialog :show-confirm-button="false" v-model:show="showCode">
      <van-icon name="cross" class="close" @click="closeCode" />
      <div>微信支付</div>
      <van-image width="150" height="150" :src="codeImage" />
      <div>请使用本人微信扫描二维码完成支付</div>
    </van-dialog>
  </div>
</template>

<style lang="less" scoped>
.container {
  min-height: 100vh;
  padding-bottom: 24px;
  background-color: #f0f0f0;
}

.header {
  padding: 10px 0;
  text-align: center;
  line-height: 30px;
  font-size: 15px;
  background-color: #fff;

  .header-left {
    float: left;
  }
}

.cell {
  width: 95%;
  margin: 8px auto;
  background-color: #fff;

  ::v-deep(.van-field__control) {
    color: var(--van-cell-value-color);
  }

  ::v-deep(.van-cell__title) {
    display: flex;
    align-items: center;
  }

  .server-name {
    margin-left: 10px;
  }
}

.service-text {
  background: url(data:image/png;base64,)
    no-repeat center center;
  background-size: 20px;
}

.submit {
  display: block;
  width: 95%;
  margin: 14px auto 0;
  box-sizing: border-box;
}

::v-deep(.van-dialog__content) {
  text-align: center;
  padding: 20px;

  .close {
    position: absolute;
    left: 20px;
  }
}
</style>
