...
onMounted(async () => {
  ...
  console.log()
  Object.assign(homeData.value, data.data.data)
...
<template>
  ...
  <van-row justify="space-around">
    ...
    >
      <van-image :src="item.pic_image_url" class="center-image"></van-image>
    </van-col>
  </van-row>

  <van-row @click="goOrder(item)" class="yy-list" v-for="item in homeData.hospitals" :key="item.id">
    <van-col span="6">
      <van-image :src="item.avatar_url" width="100" height="90"></van-image>
    </van-col>
...
.header {
  ...
  :deep(.header-search.van-search) {
    padding: 0;
  }
  :deep(.header-search .van-search__content) {
    height: 40px;
  }
}
.my-swiper {
