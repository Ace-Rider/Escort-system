...
const second = computed(() => {
  return detailData.value.order_start_time
    ? detailData.value.order_start_time + 7200000 - Date.now()
    : 0
})
...
onMounted(async () => {
  ...
  Object.assign(detailData.value, data.data.data)
  console.log(data.data)
  // console.log(detailData.value)
  if (detailData.value.trade_state === '待支付') {
    qrcode.toDataURL(detailData.value.code_url).then((res) => {
      codeImage.value = res
    })
  }
  console.log(detailData.value)
  ...
  <div class="container">
    ...
    <div class="tips">
      <div class="dzf" v-if="detailData.trade_state === '待支付'">
        ...
        <div class="text2">请在<counter :second="second"></counter>后完成支付,超时订单自动取消</div>
        <div class="text3">
          <van-button type="success" @click="showCode = true">立即支付</van-button>
        </div>
      </div>
      <div class="dfw" v-if="detailData.trade_state === '待服务'">
      ...
      <div class="ywc" v-if="detailData.trade_state === '已完成'">
        ...
        <div class="text2">感谢您的使用，如有售后问题，请联系客服</div>
      </div>
      <div class="yqx" v-if="detailData.trade_state === '已取消'">
        <div class="text1">订单已取消</div>
        <div class="text2">期待下次为您服务，如需帮助，可咨询客服</div>
      </div>
    </div>
    ...
    <van-dialog :show-confirm-button="false" v-model:show="showCode">
      ...
      <div>微信支付</div>
      <van-image width="150" height="150" :src="codeImage"></van-image>
